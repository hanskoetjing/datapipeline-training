...
@dag(
    dag_id            = "load_products_from_file",
...
def main():
    @task
    def load_file_products(**kwargs):
        xcom_store = kwargs["ti"]
        products_raw_path = Path.cwd() / "data" / "raw" / "raw_csv_file" / "products" 
        products_done_path = Path.cwd() / "data" / "raw" / "raw_csv_file" / "products" / "done"
        if (not(products_done_path.is_dir())):
            products_done_path.mkdir()
        products_raw_files = glob(str(products_raw_path / "*.csv"))
        loaded_data = None
        for file_obj in products_raw_files:
            loaded_data = []
            file_obj_split = file_obj.split('/')
            file_name = file_obj_split[-1]
            raw_file_path = products_raw_path / file_name
            ...
            products_done_file = products_done_path / file_name
            raw_file_path.rename(products_done_file)
        print(loaded_data)
