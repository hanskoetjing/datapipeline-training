...
@dag(
    dag_id            = "load_employees_from_file",
...
def main():
    @task
    def load_file_employees(**kwargs):
        xcom_store = kwargs["ti"]
        employees_raw_path = Path.cwd() / "data" / "raw" / "raw_csv_file" / "employees" 
        employees_done_path = Path.cwd() / "data" / "raw" / "raw_csv_file" / "employees" / "done"
        if (not(employees_done_path.is_dir())):
            employees_done_path.mkdir()
        employees_raw_files = glob(str(employees_raw_path / "*.csv"))
        loaded_data = None
        for file_obj in employees_raw_files:
            loaded_data = []
            file_obj_split = file_obj.split('/')
            file_name = file_obj_split[-1]
            raw_file_path = employees_raw_path / file_name
            ...
            employees_done_file = employees_done_path / file_name
            raw_file_path.rename(employees_done_file)
        print(loaded_data)
