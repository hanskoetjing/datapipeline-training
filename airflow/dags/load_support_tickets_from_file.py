...
@dag(
    dag_id            = "load_support_tickets_from_file",
...
def main():
    @task
    def load_file_support_tickets(**kwargs):
        ...
        loaded_data = None
        for file_obj in support_tickets_raw_files:
            loaded_data = []
            file_obj_split = file_obj.split('/')
            file_name = file_obj_split[-1]
            raw_file_path = support_tickets_raw_path / file_name
            ...
            support_tickets_done_file = support_tickets_done_path / file_name
            raw_file_path.rename(support_tickets_done_file)
        print(loaded_data)
    ...
    def db_access(**kwargs):
        ...
        data = xcom_store.xcom_pull(
            task_ids = "load_file_support_tickets",
            key      = "data"
        )
