...
@dag(
    dag_id            = "load_order_items_from_file",
...
def main():
    @task
    def load_file_order_items(**kwargs):
        ...
        loaded_data = None
        for file_obj in order_items_raw_files:
            loaded_data = []
            file_obj_split = file_obj.split('/')
            file_name = file_obj_split[-1]
            raw_file_path = order_items_raw_path / file_name
            ...
            order_items_done_file = order_items_done_path / file_name
            raw_file_path.rename(order_items_done_file)
        print(loaded_data)
    ...
    def db_access(**kwargs):
        ...
        data = xcom_store.xcom_pull(
            task_ids = "load_file_order_items",
            key      = "data"
        )
        ...
        if data != None:
            ...
            with postgres_hook.get_conn() as db_conn:
                target_table = "bronze.order_items"
                fields = data[0]
                data.pop(0)
                rows = data
