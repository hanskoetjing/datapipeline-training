...
def main():
    @task
    def load_file_orders(**kwargs):
        xcom_store = kwargs["ti"]
        orders_raw_path = Path.cwd() / "data" / "raw" / "raw_csv_file" / "orders" 
        orders_done_path = Path.cwd() / "data" / "raw" / "raw_csv_file" / "orders" / "done"
        if (not(orders_raw_path.is_dir())):
            print("No order data available")
            return None
        if (not(orders_done_path.is_dir())):
            orders_done_path.mkdir()
        orders_raw_files = glob(str(orders_raw_path / "*.csv"))
        loaded_data = None
        for file_obj in orders_raw_files:
            loaded_data = []
            file_obj_split = file_obj.split('/')
            file_name = file_obj_split[-1]
            raw_file_path = orders_raw_path / file_name
            ...
            orders_done_file = orders_done_path / file_name
            raw_file_path.rename(orders_done_file)
        print(loaded_data)
