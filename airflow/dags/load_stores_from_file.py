...
def main():
    @task
    def load_file_stores(**kwargs):
        xcom_store = kwargs["ti"]
        stores_raw_path = Path.cwd() / "data" / "raw" / "raw_csv_file" / "stores" 
        stores_done_path = Path.cwd() / "data" / "raw" / "raw_csv_file" / "stores" / "done"
        if (not(stores_done_path.is_dir())):
            stores_done_path.mkdir()
        stores_raw_files = glob(str(stores_raw_path / "*.csv"))
        loaded_data = None
        for file_obj in stores_raw_files:
            loaded_data = []
            file_obj_split = file_obj.split('/')
            file_name = file_obj_split[-1]
            raw_file_path = stores_raw_path / file_name
            ...
            stores_done_file = stores_done_path / file_name
            raw_file_path.rename(stores_done_file)
        print(loaded_data)
